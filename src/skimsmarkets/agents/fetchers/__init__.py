...
Each provider's `fetch(event, lens, *, lens_set)` runs evidence-gathering
with its native search + code-execution tools and emits a `LensNotebook`
(free-form prose + citations + computed numbers). No probability, no
signed shift, no directional verdict — those land in Stage B
(`agents/reasoners.py`).
...
- `render_user_message_for_lens` — composes the per-lens user message
  used by both providers (event context + sport hint + render_extras).
"""
