...
Scope: `skims execute` only. Polymarket is the data source; the trader
discovers Kalshi events at trade time via `list_events` + matches the
predicted winner against a `KalshiMarket` by surname, then signs a
`/portfolio/orders` POST. Slate-side fields (book size, OI, volume,
tipoff, tournament metadata) lived on these models only for the brief
Kalshi-as-slate-source era and were dropped when Polymarket reclaimed
the data path.

Money fields ship as strings on the public read paths (`"0.1700"`) and
as integer cents on the order paths. The matcher reads dollar floats;
the order code writes cents.

Kalshi rotates field names without a versioning policy; `extra="ignore"`
on every model keeps the parse forward-compatible.
"""
...
class KalshiMarket(BaseModel):
    ...
    last-token substring. The trader then places the order against
    `ticker` at a `yes_ask_dollars` ceiling.
    """
    ...
    # Public-read price field ships as a string ("0.1700"); coerce to
    # float so the trader can do arithmetic without unwrapping. Missing
    # / unparseable → None, which the trader treats as "no ask, skip".
    yes_ask_dollars: float | None = None
    # `status` gates tradability — only `"active"` markets pass the
    # trader's `market_closed` check.
    status: str | None = None

    @field_validator("yes_ask_dollars", mode="before")
    @classmethod
...
class KalshiEvent(BaseModel):
    ...
    no tournament prefix, no first names. The matcher hits `title`
    for both last names and resolves to the YES `KalshiMarket` by
    winner surname.
    """
